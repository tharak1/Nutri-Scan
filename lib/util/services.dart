...
void addNewProduct({required String name, required String barcode}) async {
...
Future<String?> extractIngredient({required String barcode}) async {
  // a registered user login for https://world.openfoodfacts.org/ is required
  User myUser = User(userId: 'tharak2', password: '#Since2004');
  debugPrint("HIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII");
  // query the OpenFoodFacts API
  OcrIngredientsResult response = await OpenFoodAPIClient.extractIngredients(
      myUser, barcode, OpenFoodFactsLanguage.ENGLISH);
...
void saveAndExtractIngredient() async {
  ...
  OcrIngredientsResult ocrResponse = await OpenFoodAPIClient.extractIngredients(
      myUser, '3613042717385', OpenFoodFactsLanguage.ENGLISH);
...
void addIngredientsToProduct(
    {required String ingredients, required String barcode}) async {
  // define the product to be added.
  // more attributes available ...
  Product myProduct = Product(
    barcode: barcode,
